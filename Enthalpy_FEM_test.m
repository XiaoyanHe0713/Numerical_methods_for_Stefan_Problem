...
% Physical parameters
rho = 1.0;    % density
cp  = 1.0;    % specific heat (renamed to avoid conflict)
kappa = 1.0;  % thermal conductivity (renamed to avoid conflict)
Latent = 1.0; % latent heat per unit mass (renamed to avoid conflict)
...
dt = 2.5252e-06;  % Smaller time step for stability
t_final = 0.1;
...
for n = 1:steps
    ...
    % Explicit update: dH/dt = -kappa * M^{-1} * K * T
    dHdt = -(kappa / rho) * (M_lumped \ (K * T));
    
    % Update enthalpy at free nodes only
    H(free_dofs) = H(free_dofs) + dt * dHdt(free_dofs);
    
    % Update temperature from enthalpy
    % Solid: H <= cp*Tm (which is 0 in this case)
    % Phase change: cp*Tm < H < cp*Tm + Latent
